...
export function WelcomeBanner({ onTryDemo, onDismiss }: WelcomeBannerProps) {
  return (
    <div className="rounded-xl border border-chess-accent/25 bg-chess-accent/5 p-4 mb-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-chess-muted mt-1 leading-relaxed">
            Paste a Lichess or Chess.com link, or try a famous game to see move classifications and accuracy.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        ...
        <button
          type="button"
          onClick={onDismiss}
          className="text-xs px-3 py-1.5 rounded-lg border border-chess-border text-chess-muted hover:text-chess-text"
        >
          Got it
        </button>
      </div>
