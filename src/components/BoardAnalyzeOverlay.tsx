    ...
    return (
      <div
        ...
        aria-label={
            ...
            ? `Previous review of ${conflict.runningLabel} is still open. Analyze this game, or open that review.`
      ...
      >
        <div className="analyze-now-plaque pointer-events-auto flex flex-col items-stretch gap-3.5 rounded-xl border border-chess-border bg-chess-panel/95 backdrop-blur-sm px-5 py-4 shadow-[0_18px_56px_rgba(0,0,0,0.7)] min-w-[248px] max-w-[300px]">
          ...
          {done ? (
            <>
              {/* Current board is primary — people assume the big button is for this game. */}
              <button
                type="button"
              ...
              <button
                ...
                className="flex flex-col items-center justify-center gap-0.5 rounded-lg border border-chess-border-strong bg-chess-surface px-2 py-2 text-center transition-colors hover:border-chess-accent/40 hover:text-chess-text"
              >
                <span className="text-[12px] font-semibold text-chess-subtext">
                  Open previous review
                </span>
                <span className="max-w-full truncate text-[10px] font-medium text-chess-muted">
                  {conflict.runningLabel}
                </span>
              </button>
            </>
