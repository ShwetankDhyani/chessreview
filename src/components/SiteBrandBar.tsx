...
function BrandMark() {
  return (
    <>
      <span
        className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-chess-accent/30 to-chess-accent/[0.05] border border-chess-accent/40 text-chess-accent select-none shadow-rim transition-colors duration-200 ease-soft group-hover:border-chess-accent/60"
        aria-hidden
      >
      ...
      <span className="font-extrabold text-[18px] tracking-[-0.03em] leading-none inline-flex items-baseline">
...
const navClass = ({ isActive }: { isActive: boolean }) =>
  `relative inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[12px] font-semibold tracking-wide transition-colors duration-200 ease-soft ${
    isActive
...
export function SiteBrandBar({
  ...
  const brandInner = (
    <span className="group flex items-center gap-2.5 min-w-0 flex-shrink-0">
      <BrandMark />
    </span>
  );
  ...
  return (
    <header className="relative z-50 flex flex-shrink-0 items-center gap-2 sm:gap-3 page-inline-pad min-h-[var(--app-header-h)] py-2.5 bg-chess-panel/95 backdrop-blur-md shadow-elev-1 after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-gradient-to-r after:from-transparent after:via-chess-accent/35 after:to-transparent">
      ...
      <nav
        className="flex items-center gap-0.5 sm:gap-1 ml-1 min-w-0"
        aria-label="Product"
      >
