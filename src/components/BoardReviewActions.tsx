...
/** Compact re-analyze + save controls shown under the board (desktop) or inline (mobile). */
export function BoardReviewActions({
  ...
  if (!canReanalyze && !canSave && !saveMessage) return null;
  ...
  if (inline) {
    if (!canReanalyze && !canSave) return null;
