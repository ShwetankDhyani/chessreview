...
describe("buildReviewStory", () => {
  it("picks winner's decisive blunder over earlier opponent slip", () => {
    ...
    moves[16] = stubMove(16, {
      ...
      evalAfter: { mate: -1, depth: 12, source: "local" },
    });
  ...
  it("picks mate-forcing blunder over smaller earlier mistake", () => {
    const summary: ReviewSummary = {
      white: {
        brilliant: 0,
        great: 0,
    ...
    const text = segmentText(
      buildReviewStory(summary, moves, "DrCarlss", "MrDhyani", "*").lines
    );
    expect(text).toMatch(/Nge2/);
    expect(text).not.toMatch(/Bc5/);
  });
