...
/** Expected-points loss bands (Lichess-style: ~10% / ~20% for inaccuracy / mistake). */
...
/**
 * Centipawn thresholds — blunder needs a visible eval-bar swing, not noise vs best line alone.
 */
...
export const WP_SWING_BLUNDER = 0.22;
export const WP_SWING_COMBINED = 0.14;
...
/**
 * True blunder: eval bar / win-% move a lot. Small ep or cp vs best alone stays mistake.
 */
export function isGameChangingBlunder(input: {
  ...
  if (
    input.cpLossVsBest >= CP_THRESHOLDS.combinedVsBest &&
    wpSwing >= WP_SWING_COMBINED
  ) {
    return true;
  }

  if (
    input.cpSwing >= CP_THRESHOLDS.combinedSwing &&
    wpSwing >= WP_SWING_COMBINED
  ) {
    return true;
  }

  if (wpBefore >= 0.68 && wpAfter <= 0.35 && wpSwing >= 0.2) return true;
...
export function classifyMove(input: ClassifyMoveInput): MoveClassification {
  ...
  const gameChanging = isGameChangingBlunder({
    epLoss,
    cpLossVsBest,
    cpSwing,
    wpBeforePct,
  ...
  if (gameChanging) return "blunder";
