...
export function getSquareCenter(
  ...
  const pos = squareGridPos(square, boardOrientation);
  if (!pos) {
    return { x: boardWidth / 2, y: boardWidth / 2 };
  }

  const sq = boardWidth / 8;
  return { x: pos.col * sq + sq / 2, y: pos.row * sq + sq / 2 };
}

/** Top-left pixel rect of a square, relative to the top-left of the board. */
export function getSquareRect(
  square: string,
  boardOrientation: "white" | "black",
  boardWidth: number
  ...
  return { left: pos.col * size, top: pos.row * size, size };
}
