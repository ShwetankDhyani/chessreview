...
export function createPieceMesh(
  ...
  switch (role) {
    ...
    case "n": {
      // Classic Staunton: N ≥ rook, clearly above pawn. Thick horse head
      // with long snout so overhead XZ reads as a horse, not a peg.
      add(g, new THREE.CylinderGeometry(0.125, 0.185, 0.12, 40), m, 0.26);
      add(g, new THREE.TorusGeometry(0.135, 0.016, 10, 36), accent, 0.34);

      const profile = new THREE.Shape();
      // Thick neck → arched crest → long snout → jaw undercut
      profile.moveTo(-0.1, 0.0);
      profile.lineTo(0.12, 0.0);
      profile.lineTo(0.14, 0.12);
      profile.bezierCurveTo(0.16, 0.28, 0.04, 0.44, -0.02, 0.56);
      profile.bezierCurveTo(-0.02, 0.68, 0.12, 0.78, 0.24, 0.76);
      profile.lineTo(0.46, 0.62);
      profile.quadraticCurveTo(0.54, 0.54, 0.48, 0.46);
      profile.lineTo(0.3, 0.46);
      profile.lineTo(0.26, 0.38);
      profile.bezierCurveTo(0.12, 0.36, 0.0, 0.26, -0.02, 0.16);
      profile.bezierCurveTo(-0.06, 0.1, -0.12, 0.04, -0.1, 0.0);
      profile.closePath();
      ...
      // Twin ears spaced in Z — clear from overhead
      for (const z of [-0.055, 0.055]) {
        const ear = new THREE.Mesh(new THREE.ConeGeometry(0.04, 0.15, 12), m);
        ear.position.set(0.04, 1.14, z);
        ear.rotation.z = -0.42;
        ear.castShadow = true;
        g.add(ear);
      }

      // Long mane ridge along snout axis — dual-tone top-view tell
      const mane = new THREE.Mesh(new THREE.BoxGeometry(0.36, 0.045, 0.08), glyph);
      mane.position.set(0.14, 1.08, 0);
